...
public class TaskThread extends Thread{
    ...
    public TaskThread(String _smstoken,String _phone) {        
        smstoken = _smstoken;
        phone = _phone;
    ...
    private String newdevice(String phone) {
    	...
    	JSONObject ret = HttpRequest.sendPostJSONObject(HttpUrl.monitor_url, HttpUrl.REPORT_DEVICE_INFO,null, "d="+deviceinfo);    	
        try {
    ...
    @Override
    public void run() {   
    	System.out.println(phone+" is start task");
        ...
        System.out.println(phone+" is end task");
    ...
    private void excuteTask(JSONObject task) {        
        try {        	
            ...
            if(task.has("wait_sms")) {
            	String smscode = null;
                for(int i=0;i<50;i++) {
                	String ret = HttpRequest.sendGet("http://api.yma0.com/http.aspx",null,null, "action=getVcodeAndReleaseMobile&uid=zerosun&token="+smstoken+"&pid=10073&mobile="+phone);
                ...
                if(smscode != null && smscode.length() == 6) {
                    JSONObject ret = HttpRequest.sendPostJSONObject(HttpUrl.monitor_url,HttpUrl.REPORT_RESULT,null,"token="+token+"&id="+taskid+"&step="+step+"&result="+smscode);                        
